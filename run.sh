...
script -c "time -f '\t%e real,\t%U user,\t%S sys' python tf_cnn_benchmarks.py --num_gpus=4 --batch_size=32 --use_fp16 --model=resnet50 --variable_update=parameter_server" /workspace/logs/benchmark_resnet50_synthetic.log
if [ -d "/tfrecords" ] ; then
  script -c "time -f '\t%e real,\t%U user,\t%S sys' python tf_cnn_benchmarks.py --data_format=NCHW --batch_size=32 --model=resnet50 --optimizer=momentum --variable_update=replicated --nodistortions --gradient_repacking=8 --num_gpus=4 --num_epochs=90 --weight_decay=1e-4 --data_dir=/tfrecords/ --data_name=imagenet --use_fp16 --train_dir=/workspace/resnet50_train" /workspace/logs/benchmark_resnet50_imagenet.log
fi
